...
def call(
    ...
    try:
        ...
        try:
            ...
            body = {}
        message = body.get("message") or f"HTTP {err.code}"
        raise SupervisorError(f"{method} {path}: {message}") from err
    except urllib.error.URLError as err:
        raise SupervisorError(f"{method} {path}: {err.reason}") from err

    if body.get("result") == "error":
        raise SupervisorError(f"{method} {path}: {body.get('message', 'unknown')}")
...
def store_addon(slug: str) -> dict[str, Any] | None:
    """Return the store entry of an app, or None when it is unknown."""
    try:
...
def addon_info(slug: str) -> dict[str, Any] | None:
    """Return the info of an installed app, or None when it is not installed."""
    try:
...
def partial_backup(name: str, slug: str) -> str:
