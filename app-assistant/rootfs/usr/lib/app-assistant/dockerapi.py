...
def exec_in_supervisor(command: str | list[str], timeout: int = 900) -> tuple[int, str]:
    ...
    if result.get("Running") or not isinstance(result.get("ExitCode"), int):
        raise DockerError("Command completion is not confirmed")
